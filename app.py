...
# -------------------------------------------------------------------------
# PANEL DE ADMINISTRACIÓN (SOLO PARA VOS)
# -------------------------------------------------------------------------
with st.sidebar:
    st.header("⚙️ Panel de Control")
    st.write("Este apartado es exclusivo para el Administrador.")
    ...
    if os.path.exists(ARCHIVO_CSV):
        ...
        # PIN de seguridad simple para que ningún compañero borre datos por error si descubre la barra lateral
        pin_seguridad = st.text_input("Clave de Admin para borrar", type="password")
        if st.button("🗑️ Borrar Semana Actual", type="primary", use_container_width=True):
            if pin_seguridad == "admin123":  # Podés cambiar "admin123" por la clave que quieras
                if os.path.exists(ARCHIVO_CSV): os.remove(ARCHIVO_CSV)
                if os.path.exists(PLANTILLA_EXCEL): os.remove(PLANTILLA_EXCEL)
                st.success("Semana borrada. El sistema vuelve a estar en espera.")
                st.rerun()
            else:
                st.error("❌ Clave incorrecta. No tienes permisos.")

# -------------------------------------------------------------------------
# PANEL DE TRABAJO (LO QUE VEN LOS MUCHACHOS)
# -------------------------------------------------------------------------
            ...
            if st.button("🚀 Generar Reporte Oficial (.xlsx)", use_container_width=True):
                try:
                    ...
                    if idx_sitio is None:
                        st.error("No se pudo encontrar la columna SITIO.")
                    else:
                        ...
                        for f in range(fila_titulos + 1, ws.max_row + 1):
                            ...
                            if cod_sitio in mapeo_realizo:
                                ...
                                celda_r.border = borde_cuadrícula
                                celda_j.border = borde_cuadrícula
                                
                                celda_base = ws.cell(row=f, column=idx_sitio)
                                if celda_base.has_style:
                                    celda_r.font = Font(name=celda_base.font.name, size=celda_base.font.size, bold=celda_base.font.bold, color=celda_base.font.color)
                                    celda_j.font = Font(name=celda_base.font.name, size=celda_base.font.size, bold=celda_base.font.bold, color=celda_base.font.color)
                        ...
                        st.success("¡Archivo oficial listo!")
