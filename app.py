        ...
        if nuevo_excel_main is not None:
            if st.button("🔄 Inicializar Nueva Semana", use_container_width=True, key="btn_init_main"):
                try:
                    ...
                    if c_re: 
                        df_c = df_c.rename(columns={c_re[0]: "Se realizó (Si/No)"})
                    else: 
                        df_c["Se realizó (Si/No)"] = "Pendiente"
                            
                    if c_ju: 
                        df_c = df_c.rename(columns={c_ju[0]: "Si no se realizó detallar el por qué."})
                    else: 
                        df_c["Si no se realizó detallar el por qué."] = ""
                    
                    df_c.loc[df_c["Se realizó (Si/No)"] == "", "Se realizó (Si/No)"] = "Pendiente"
                    if "SITIO" in df_c.columns: 
        ...
        with col2:
            if st.button("🚀 Generar Reporte Oficial (.xlsx)", use_container_width=True):
                try:
                    ...
                    # 1. Encontrar la fila exacta de los encabezados reales
                    for r in range(1, 15):
                    ...
                    if idx_s is None:
                        ...
                        # Si las columnas no existen, las asignamos en una posición lógica para no duplicar
                        if idx_r is None: 
                            idx_r = ws.max_column + 1
                        ...
                        # Mapear los datos que los muchachos cambiaron en la app web
                        ...
                        # 2. Inyectar los datos en el clon respetando celdas originales
                        for f in range(fila_t + 1, ws.max_row + 1):
                            ...
                            if cod_s in map_r:
                                # Reemplazamos directo sobre las columnas existentes
                                cell_realizado = ws.cell(row=f, column=idx_r)
                                cell_justificacion = ws.cell(row=f, column=idx_j)
                                
                                cell_realizado.value = map_r[cod_s]
                                cell_justificacion.value = map_j[cod_s]
                                
                                # Aplicamos bordes oficiales
                                cell_realizado.border = borde_fino
                                cell_justificacion.border = borde_fino
                                
                                # Copiar estilos de texto de la columna de SITIO (Misma fuente y tamaño)
                                c_base = ws.cell(row=f, column=idx_s)
                                if c_base.has_style and c_base.font:
                                    font_oficial = Font(name=c_base.font.name, size=c_base.font.size, bold=c_base.font.bold, color=c_b.font.color if hasattr(c_base.font, 'color') else None)
                                    cell_realizado.font = font_oficial
                                    cell_justificacion.font = font_oficial
                                
                                # Formato de alineación limpia
                                cell_realizado.alignment = Alignment(horizontal="center", vertical="center")
                                cell_justificacion.alignment = Alignment(horizontal="left", vertical="center")
                        ...
                        st.success("¡Reporte alineado correctamente!")
                except Exception as ex:
                    st.error(f"Error al procesar el Excel: {ex}")
    except Exception as e:
