...
if os.path.exists(ARCHIVO_SERVIDOR):
    try:
        ...
        with col2:
            # CORRECCIÓN DE EXPORTACIÓN: Convertir los datos a un archivo Excel (.xlsx) en memoria
            buffer_excel = io.BytesIO()
            with pd.ExcelWriter(buffer_excel, engine='openpyxl') as writer:
                # Se guarda sin el índice numérico para que quede idéntico al original
                df_editado.to_excel(writer, sheet_name=f"Semana {num_semana}", index=False)
            ...
            st.download_button(
                ...
                data=data_excel,
                file_name=f"Reporte_Final_Semana_{num_semana}.xlsx",
