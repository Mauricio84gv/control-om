        ...
        if st.button("🔄 Inicializar Semana", use_container_width=True):
            try:
                ...
                if col_justif:
                    ...
                    # Si venían columnas de comentarios extras o repetidas por errores anteriores, las limpiamos del dataframe web
                    for extra_col in col_justif[1:]:
                        if extra_col in df_cargado.columns:
                            df_cargado = df_cargado.drop(columns=[extra_col])
                else:
                ...
                # Eliminar duplicados si la oficina mandó datos repetidos
...
if os.path.exists(ARCHIVO_CSV) and os.path.exists(PLANTILLA_EXCEL):
    try:
        ...
        with col2:
            # PROCESO DE INYECCIÓN INTELIGENTE (ESTILOS Y COLUMNAS CORREGIDAS)
                ...
                try:
                    ...
                    if idx_sitio is None:
                        ...
                        if idx_justif is None:
                            ...
                            # Asegurar el texto exacto en el encabezado original de la plantilla
                            ws.cell(row=fila_titulos, column=idx_justif, value="Si no se realizó detallar el por qué.")
                        ...
                        # Definición estricta de bordes delgados negros tipo rejilla de Excel
                        ...
                        # Recorrer filas e inyectar aplicando la rejilla oficial
                        for f in range(fila_titulos + 1, ws.max_row + 1):
                            ...
                            if cod_sitio in mapeo_realizo:
                                ...
                                # Aplicar bordes de rejilla obligatorios a ambas columnas
                                celda_r.border = borde_cuadrícula
                                celda_j.border = borde_cuadrícula
                                
                                # Clonar fuente de la celda de la par (SITIO) para homogeneidad
