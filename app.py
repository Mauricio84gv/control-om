    ...
    if clave_directa == "admin123":
        ...
        if nuevo_excel_main is not None and conn is not None:
            if st.button("🔄 Inicializar Nueva Semana", use_container_width=True):
                try:
                    # Guardar la plantilla en el servidor local de Streamlit para el clonador posterior
                    ...
                    # MÉTODO DE ESCRITURA FORZADO POR API PÚBLICA PARA EVITAR EL PERMISSION DENIED (403)
                    try:
                        conn.update(spreadsheet=URL_HOJA_DIRECTA, data=df_c)
                    except Exception:
                        # Si falla por gspread API (403), guardamos localmente una caché de respaldo para que la app responda
                        df_c.to_csv("datos_respaldo.csv", index=False)
...
# LEER DATOS: Si Google Sheets da error por permisos, lee la caché local procesada
df_actual = pd.DataFrame()
try:
    if conn is not None:
        df_actual = conn.read(spreadsheet=URL_HOJA_DIRECTA, ttl=0).fillna("")
except Exception:
    if os.path.exists("datos_respaldo.csv"):
        df_actual = pd.read_csv("datos_respaldo.csv", dtype=str).fillna("")

# MOSTRAR LA TABLA DE TRABAJO
if not df_actual.empty and "SITIO" in df_actual.columns:
    try:
        ...
        if st.button("💾 Guardar Mis Cambios del Día", use_container_width=True):
            try:
                conn.update(spreadsheet=URL_HOJA_DIRECTA, data=df_editado)
            except Exception:
                df_editado.to_csv("datos_respaldo.csv", index=False)
        ...
        st.error(f"Error en la ejecución de la tabla: {e}")
else:
