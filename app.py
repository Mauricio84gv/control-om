...
with st.sidebar:
    ...
    if nuevo_excel is not None:
        if st.button("🔄 Inicializar Semana", use_container_width=True):
            try:
                # Leer el archivo Excel forzando todo a texto para evitar problemas con notas o celdas mezcladas
                df_cargado = pd.read_excel(nuevo_excel, dtype=str)
                df_cargado = df_cargado.fillna("")
                
                # LIMPIEZA INTELIGENTE DE COLUMNAS:
                # Pasamos todos los encabezados a MAYÚSCULAS y quitamos espacios fantasmas
                columnas_limpias = []
                for col in df_cargado.columns:
                    nombre_limpio = str(col).strip().upper()
                    # Si dice algo similar a SITIO, lo unificamos
                ...
                # Crear o unificar las columnas de campo que usarán tus compañeros
                # Buscamos variaciones para no duplicar si ya venían en el archivo
                ...
                # Asegurar que los estados vacíos inicien como "Pendiente"
                df_cargado.loc[df_cargado["Se realizó (Si/No)"] == "", "Se realizó (Si/No)"] = "Pendiente"

                # Limpieza de espacios fantasmas dentro de las celdas de datos
                for col in df_cargado.columns:
                ...
                st.success("¡Lista semanal procesada con éxito!")
                st.rerun()
...
if os.path.exists(ARCHIVO_SERVIDOR):
    try:
        ...
        # Intentar obtener la semana de forma segura
        ...
        df_editado = st.data_editor(
            ...
            # Protegemos todos los datos de origen, solo dejamos editar las columnas de campo
